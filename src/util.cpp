...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "BadCoin.conf"));
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    {
        // Don't overwrite existing settings so command line settings override BadCoin.conf
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "badcoind.pid"));
    ...
    if (vTimeOffsets.size() >= 5 && vTimeOffsets.size() % 2 == 1)
    {
        ...
        else
        {
            ...
            if (!fDone)
            {
                ...
                {
                    fDone = true;
                    string strMessage = _("Warning: Please check that your computer's date and time are correct! If your clock is wrong Badcoin will not work properly.");
