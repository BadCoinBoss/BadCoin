...
uint256 hashGenesisBlock("0x000000004a29a6f760afaeb95074017278ff904b11475429ba7e4234263b3f45");
...
const string strMessageMagic = "Badcoin Signed Message:\n";
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    // Subsidy is cut in half every 34560 blocks, which will occur approximately every 4 years
    nSubsidy >>= (nHeight / 34560);

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan2 = 300;
static const int64 nTargetSpacing = 300;
static const int64 nInterval2 = nTargetTimespan2 / nTargetSpacing;
static const int64 nAdjustDiffMaxUp2 = 2;
static const int64 nAdjustDiffMaxDown = 4;

static const int64 nTTChangeHeight = 15000;
static const int64 nAdjustDiffMaxUpOriginal = 4;
static const int64 nTargetTimespanOriginal = 18000;
static const int64 nIntervalOriginal = nTargetTimespanOriginal / nTargetSpacing;
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    int64 nTargetTimespan = nTargetTimespan2; 
	int64 nAdjustDiffMaxUp = nAdjustDiffMaxUp2;
	int64 nInterval = nInterval2;
		
    if (pindexLast->nHeight < nTTChangeHeight)
	{
    nTargetTimespan = nTargetTimespanOriginal; 
	nAdjustDiffMaxUp = nAdjustDiffMaxUpOriginal;
	nInterval = nIntervalOriginal;
	}
    ...
    if (nActualTimespan < nTargetTimespan/nAdjustDiffMaxDown)
        nActualTimespan = nTargetTimespan/nAdjustDiffMaxDown;
    if (nActualTimespan > nTargetTimespan*nAdjustDiffMaxUp)
        nActualTimespan = nTargetTimespan*nAdjustDiffMaxUp;
    ...
    printf("nTargetTimespan = %"PRI64d"    nActualTimespan = %"PRI64d"	 nAdjustDiffMaxUp = %"PRI64d"	nInterval = %"PRI64d"\n", nTargetTimespan, nActualTimespan, nAdjustDiffMaxUp, nInterval);
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x000000004a29a6f760afaeb95074017278ff904b11475429ba7e4234263b3f45");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        block.nVersion = 1;
        block.nTime    = 1415050369;
        block.nBits    = 486604799;
        block.nNonce   = 3913867835;
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("BadcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("BadcoinMiner : ProcessBlock, block not accepted");
    }
...
void static BitcoinMiner(CWallet *pwallet)
{
    ...
    try { loop {
        ...
        printf("Running BadcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("BadcoinMiner terminated\n");
        throw;
    }
}
